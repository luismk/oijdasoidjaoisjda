...
namespace UGPangya.MessengerServer
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            _messengerserver = new MessengerServerTcp("149.56.33.85", 30303, 3000);
            _messengerserver.OnClientConnected += player =>
        ...
        private static void TcpServer_OnPacketReceived(Player playerType, Packet packet)
        {
            var player = (MPlayer) playerType;

            Console.WriteLine(DateTime.Now + $" PACKET [{player.CurrentPacket.Id}]: " +
                              (MessengePacketEnum) player.CurrentPacket.Id);
