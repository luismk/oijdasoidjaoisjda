...
namespace UGPangya.API.Repository
{
    public class GuildRepository
    {
        ...
        /// <returns>
        ///     Retorna código resultado: 10 = PLAYER IS IN GUILD,  0 = SUCCESSFULLY CREATED GUILD, 9 = GUILD NAME IS ALREADY
        ///     EXISTED, 2 = TRANSACTION ERROR
        /// </returns>
        public int CreateGuild(int uid, string guildName, string guildIntro)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                return connection.QuerySingleOrDefault<int>("USP_GUILD_CREATE",
                    new {UID = uid, GUILDNAME = guildName, GUILDINTRO = guildIntro},
                    commandType: CommandType.StoredProcedure);
            }
        }
    }
