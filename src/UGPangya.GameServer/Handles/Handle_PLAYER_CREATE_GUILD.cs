...
namespace UGPangya.GameServer.Handles
{
    public class Handle_PLAYER_CREATE_GUILD : HandleBase<Packet_PLAYER_CREATE_GUILD>
    {
        ...
        private void Handle()
        {
            var result = new GuildRepository().CreateGuild(Player.Member_Old.UID, PacketResult.GuildName,
                PacketResult.GuildInfo);
        ...
        private byte[] GuildInfo()
        {
            ...
            response.Write((uint) 0); // guild level point ok
            response.Write((uint) 0); //guild pangs OK  
            response.Write((uint) Player.Guild.GUILD_TOTAL_MEMBER); //guild total member ok
            ...
            response.Write((uint) Player.Guild.GUILD_POSITION); // Guild Position
            response.Write((uint) Player.Guild.GUILD_LEADER_UID); //// Guild Leader UID?
            response.WriteStr(Player.Guild.GUILD_LEADER_NICKNAME, 22); //guild leader nickname OK
            response.Write(
                Utils.ToPangyaDateTime(Player.Guild.GUILD_CREATE_DATE?.Date ??
                                       DateTime.Now)); //PlayerData.GuildData.GUILD_CREATE_DATE
